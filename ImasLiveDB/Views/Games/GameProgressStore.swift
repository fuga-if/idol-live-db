...
final class GameProgressStore {
    ...
    /// 公演日との比較に使う `JSTDay` とは意味が違う。
    static func today(_ date: Date = Date()) -> String { DailyPick.dayKey(date) }
    static func yesterday(_ date: Date = Date()) -> String { DailyPick.previousDayKey(date) }
}
