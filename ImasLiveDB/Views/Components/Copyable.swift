...
extension View {
    /// ネイティブの選択バー (コピー / すべてを選択 / 調べる) で**部分的に**コピーできるようにする。
    ///
    /// 使えるのは「タップ操作を持たない長文」だけ:
    /// - 行タップで遷移する一覧では使えない。選択ジェスチャがタップを吸って遷移できなくなる。
    /// - 1 行に省略される値でも使わない。画面に出ている分しか選べず、
    ///   省略された全文を取り逃す。そちらは `imasCopyable` (原文をそのまま渡す) を使う。
    ///
    /// 参考動画のメモやタグの説明のように、長くて「一部だけ引用したい」ものに向く。
    func imasSelectableText() -> some View {
