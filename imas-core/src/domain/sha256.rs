//! SHA-256 (FIPS 180-4)。
//!
//! crate を足さずに自前で持つ。用途は 2 つあり、どちらも「同じ入力なら必ず同じ値」で
//! あることだけが要る:
//!
//! * バックアップの checksum — iOS の `CryptoKit.SHA256` / Android の
//!   `MessageDigest("SHA-256")` と同じ値でないと、書き出しと検証が食い違う。
//! * `db/master.sql` の内容指紋 — `tools/build_db.sh` の `shasum -a 256` と
//!   同じ値でないと、`meta.content_hash` と突き合わせられない。
//!
//! 規格どおりに実装し、公開テストベクタで固定してある。

/// ラウンド定数 (FIPS 180-4 の K)。
const K: [u32; 64] = [
