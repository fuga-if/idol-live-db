...
`https://fujiwarahaji.me/sitemap/musiclist` に全 2,576 曲の
`https://fujiwarahaji.me/music/<ブランド>/<番号>` が載っている。ここから id を拾えば
**検索の 1 往復を省ける** (全体で半分のリクエストで済む)。
初版はこれを知らずに 1 曲ずつ検索していた。次に手を入れるならここから読むこと。
(WordPress の REST API は 403、sitemap.xml の pt-music-*.xml は 725 本に分かれていて、
どちらも使えなかった。この一覧が唯一の一括口。)
