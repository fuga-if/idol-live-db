...
**push 先は `develop`** (main は保護ブランチで bot が直接 push できないため。コントリビューターは develop 起点で
作業するので、鮮度は develop にあれば十分。main へは通常のリリースマージで反映される)。
...
2. **main の branch protection**: PR 必須 + 承認必須 + **Code Owners レビュー必須** (`.github/CODEOWNERS` の `* @owner` で全 PR をオーナー承認必須に)。
   bot は main に直接 push しない (develop 行き) ので bypass 設定は不要。

3. **CODEOWNERS** (`.github/CODEOWNERS`): `* @owner` で全ファイル + `/.github/`・`/tools/` を明示。
   secret に触れるワークフロー・反映ツールの改ざんを防ぐ。

4. **develop は緩め** (bot が `db/master.sql` を push する統合ブランチ)。保護するなら bot を許可すること。
