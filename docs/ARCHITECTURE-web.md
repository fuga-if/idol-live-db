...
- **楽曲表の列**: 曲名 (曲種別の札つき) / ユニット / 歌唱アイドル / 作曲 / 収録 CD / 初出 / ライブ。
  作曲は 76rem、収録 CD は 92rem からの列 (コンテナ幅)。素材は `SongListItem.{song_type_label,
  composer, cd_title}`。作詞・編曲は絞り込みの「作家名」で引ける。
- **列見出しで並べ替え**: `th[data-sort-column]` に Rust の `SongListSort::key` を置き、絞り込みの島
  (`listfilter/island.ts`) が起動時にボタンへ差し替える。押すと「その並びに → 向きを反転」。状態は
  select / 向きボタンと同じ 1 つで、URL (`?sort=&dir=`) にも同じく写る。島が動かない環境では文字のまま。
  `aria-sort` を見出しに立てる。
