...
- **出演者は登録分 ∪ 歌唱メンバー** (`show_cast_with_performers`)。出演者の登録が歌唱メンバーに
  追い付いていない公演が 189 あり、登録分だけで見ると「全員」が付かず人数も欠ける。
- **初披露・何回目**は `song_detail_queries::{first_performance_item_id, performance_ordinals}`。
  「この DB に載っている範囲で」の意味で、回数と同じ基準。時系列の鍵は (日付, 公演の並び, 曲順)。
- 披露履歴の行は歌唱メンバー (3 人 + ほか N 人) を持ち、公演ページの**その曲の行** (`#setlist-N`) へ
  飛ぶ。「いつ誰が歌ったか」をセトリを目で探さずに答えられるように。
- ナビは「コールガイド」(「コール」だけではアプリの外で意味が取れない)。公演ページの現在地は
  ライブ (`NavLink.match_prefixes`、所属の判断は Rust)。
