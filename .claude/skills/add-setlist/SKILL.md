...
**x.com は WebFetch できない (HTTP 402)。** `api.fxtwitter.com` 等で読める
(手順は `read-web-source` skill)。公式のセトリは**画像**なので、media URL を取って
Read ツールで開くところまでやる。
...
- **バージョン違い** — 会場ごとに歌唱メンバーが変わる曲がある
  ...
  **別公演の ver. を流用しない。** 該当 ver. は iTunes Search API で見つかることが多い
(会場ごとに Single が出る): `https://itunes.apple.com/search?term=<曲名>&country=JP&media=music&entity=song`
...
### 3. `--only` は新曲とセトリを同時に検証できない

`--only` は**全種類をファイル名で絞る**ので、セトリと同じバッチの `data/songs/` も除外される。
結果 `resolve_song` の pending が空になり、初披露曲が「曲を特定できない」で落ちる。

検証は `--only` **無し**で回して自分のファイルだけ grep する:

```bash
python3 tools/apply_data.py --check --db /tmp/cur.sqlite 2>&1 | grep -E "<自分のファイル名の共通部分>" \
  || echo "自分のファイルに問題なし"
```

`--apply` は songs → setlists の順に**ファイルを分けて 2 回**流す。

### 4. `--only` を付けないと自分の変更が届かない
