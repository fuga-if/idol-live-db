...
**このサイトは表示専用。** 担当・お気に入り・参加記録・投票・タグ・ペンライト・
編集・ログインといった「状態を持つ機能」は作らない。それらはすべて iOS/Android アプリ側にあり、
各詳細ページの「アプリで開く」導線から誘導する。

ブラウザで動く JS は**探す/絞るための島だけ**: `/search/` の横断検索と、
一覧の絞り込み・並べ替え (`src/lib/songfilter/`)。どちらも自分のオリジンに置いた
静的 JSON しか取りに行かず、API は叩かない (`tests/no-api-exposure.test.ts` が固定)。
